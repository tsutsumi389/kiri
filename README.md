...
product.png  1600x2000  png  841.4 KB  (338 ms)
  ...
  輪郭粗さ  0.00 px  (1000px 換算, 警告 0.15 超)
  縁の汚染  0.1%  (警告 1.2% 超)
...
- `rim_contamination` が 0.012 を超えれば縁に背景のテクスチャが張り付いている（警告が出る）
...
```
警告: 境界が滑らかではありません（長辺 1000px 換算で 0.18 px のギザギザ）。
      背景のテクスチャが輪郭に乗っている可能性があります
警告: 境界の内側 1.9% が、商品の色より背景の色に近いままです (rim_contamination=0.019)。
...
実写のリモコンでは `--tolerance 60`（最良）で `contour_roughness` 0.175 /
`rim_contamination` 0.019、40 まで落とすと 0.817 / 0.195 になる。
