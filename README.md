...
| `touches_edge=true` | 真偽の指標。`=false` も書ける |
...
これは `--optimize` が「きれい」と呼ぶ条件そのもので、**既に較正済みである**。
...
- キーは常に出し、当てはまらないところは `null`。固定のしきい値を持たない条件
  ...
  複合条件）では `operator` も `threshold` も `null` になる
...
| 3 | 入力ファイル異常 |
| 4 | 処理失敗 |
| 5 | 規格未達（`--fail-on` の条件に触れた）。**成果物はある。人が見る対象**で、結果 JSON は通常どおり返る |
