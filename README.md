...
| `--cleanup` | 2 | 孤立ノイズ除去の半径(px)。**長辺 1000px 換算**で指定し、面積 (2n+1)² × (長辺/1000)² 未満の連結成分を消す |
...
12MP（3000×4000）の PNG を既定オプションで切り抜いた実測は **803ms / ピーク RSS
301MB**（Apple M4 Pro、`/usr/bin/time -l`）。メモリは画素数にほぼ比例するので、
1MP なら 40MB 前後に収まる。
