...
`p90` が `--edge-threshold`（既定 8）の水準に達していれば、その背景は堤防を張れない
素材で、`cutout` はしきい値を自動で引き上げる。
...
| `--edge-threshold` | 8（自動調整あり） | 輪郭でフィルを止める勾配のしきい値。0 で無効。未指定なら背景のテクスチャに応じて自動で引き上がる |
...
**`--edge-threshold` が未指定のときに限り、しきい値を p90 の 1.5 倍まで
自動で引き上げる**。

| 背景 | 外周勾配 p90 | 効いた堤防 | 前景比率 |
|---|---|---|---|
| スタジオ背景 | 1.0 | 8（既定のまま） | 変化なし |
| 不織布（実写 20MP） | 27.9 | 41.8 | 0.94 → 0.204 |
...
```
警告: 背景のテクスチャ（外周の勾配 p90 = 27.9）が堤防を発火させるため、
      edge_threshold を 8 から 41.8 へ調整しました。明示指定すれば従います
```
