...
**外周に背景でないものが写り込んでいる画像は、この 2 つを見る前に low へ落とす。**
...
見分けは外周 ΔE の分布が二峰性かどうかで付く。`p50` < 5（9 割は文句なく単色）
かつ `p90` > 15（残り 1 割だけが明らかに別の色）なら汚染とみなす。

| シーン | p50 | p90 | 判定 |
|---|---|---|---|
| 大きい物体が画面外へ抜ける合成 | 0.0 | 28.7 | 汚染（low へ降格） |
| きれいなスタジオ背景 | 0.4 | 0.7 | 汚染ではない |
| 不織布の上のリモコン | 11.9 | 26.8 | 汚染ではない |
| 暗い机の上のキーボード | 21.6 | 61.4 | 汚染ではない（元から low） |

下 2 行が示すとおり、**`p90` が大きいこと自体は汚染ではない。** 背景全体が
ざらついていれば `p50` も一緒に上がる。二つを合わせて初めて指紋になる。

**この較正は既定の `--border 2` を前提にしている。** `--border` は背景色の推定
範囲であると同時に外周 ΔE の分布——つまり上の判定の材料そのもの——を決めるため、
大きく広げると成立しない。実測ではキーボードが `--border 110` で `high` に裏返り、
誤った矩形を勧め始める。既定から大きく動かしたときは `confidence` を根拠に
動かないこと。
