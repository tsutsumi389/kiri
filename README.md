...
| `--smooth-contour` | 2.0 | 帯の中の二値輪郭に掛けるメディアンの半径(px)。**長辺 1000px 換算**。0 で無効 |
...
1 と 2 は 4 回繰り返す。1 回で剥がせるのは帯幅ぶんだけだが、実写ではマスクが 20px 級
はみ出す場所があり、その状態では「帯の外の前景」そのものが繊維なので判定が効かない。
剥がすほど判定が効くようになる。**商品の側は削れない**——色の門が真の輪郭で止める。
...
|---|---|---|
| 不織布 + 黒商品 | 18.4 → **5.9** | 0.512 → **0.237** |
| 照明勾配の不織布 + 黒商品 | 10.2 → **1.2** | 0.256 → **0.071** |
| 不織布 + 3px ストラップ | 33.4 → **13.3** | 0.589 → **0.281** |
| 不織布 + 柔らかい輪郭 8px | 20.8 → **7.7** | 0.498 → **0.190** |
| 暗い机 + 白商品（既定値のまま） | 36.2 → **0.4** | 0.224 → **0.000** |

実写 20MP（白い不織布の上の黒いリモコン、最良設定）では `contour_roughness`
0.391 → **0.188**、`rim_contamination` 0.096 → **0.074**。納品寸法
（`--canvas 1000 --flatten`）を 4 倍に拡大すると、上辺に沿っていた明るい縁と、
その上に浮いていた灰色の粒が消える。**ただし警告は両方とも消えていない**——
`rim_contamination` が見る帯（長辺 1000px 換算の 3px ＝ 20MP では 18px）は、
境界処理が届く帯（同 3〜10px）より深い。
...
0.5 秒弱、メッシュのように構造が帯より細い素材（櫛状 6px 周期）で 2 秒弱。

#### 所要時間とメモリ

12MP（3000×4000）の PNG を既定オプションで切り抜いた実測は **1.19 秒 / ピーク RSS
338MB**（Apple M4 Pro、`/usr/bin/time -l`、3 回の最短）。内訳は `--no-refine` が
0.71 秒 / 274MB、そこへ境界のアルファの解き直しが 0.05 秒 / 16MB、帯の塗り直し
（再分類と平滑化を 4 パス）が 0.39 秒 / 43MB、guided feathering が 0.04 秒 / 5MB
を足す。メモリは画素数にほぼ比例する。
...
- `settings.band_min_radius` は**実際に効いた帯幅の下限**(px)。指定値からは
  読めない——輪郭が粗ければ粗さぶんだけ持ち上がる（`edge_threshold` の自動調整と
  同じ規約）。`--no-refine` では帯そのものが無いので**キーごと現れない**
