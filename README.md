...
| 暗い机の上のキーボード（実写） | 0.16 | 21.6 | 1.8 | 場は吸うが、商品と背景が分離できない（`NOT_SEPARABLE`） |

**`residual` が小さいことは「解ける」の十分条件ではない。** 3 行目のように、
場が背景をよく説明していても、商品と背景の色差がそもそも足りない画像はある。
`residual` は「照明を剥がしたあとに何が残るか」しか言わない。
...
双線形補間で都度引く。20MP での実測は **14.3 ms / 約 2MB**。
...
| 不織布の上のリモコン（実写 20MP） | bbox あり `--tolerance 12` | 外周接触あり / halo 17.4% | **外周接触なし / halo 15.1%** |
| 照明勾配の不織布 + 黒商品（合成 R2） | 既定 | 0.502 / 121.5 / 0.854 | 0.388 / 100.4 / 0.574（**まだ切れない**） |
...
（R2 の縁の汚染 0.854 → 0.574）が、外周接触は解消しない。
