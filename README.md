...
| `--fg-mask PATH` | — | 輝度が 0 でない画素を確定前景にするマスク画像 |
| `--bg-mask PATH` | — | 輝度が 0 でない画素を確定背景にするマスク画像 |
...
- **確定前景は色によらず守られる。** `--fg-seed` と同じ経路で、bbox の外側にも
  確定背景にも勝つ
- 4 つの入口は併用できる（和を取る）
...
（頂点が 1px はみ出しただけで指示が消えるほうが害が大きい）。
...
| `INVALID_POLYGON` | 多角形の点数か座標が不正（3 点未満、奇数個、`--normalized` で 0.0-1.0 の外） |
...
その指示は空だったということになる（真っ黒なマスク、画像の外だけを指す多角形）。
