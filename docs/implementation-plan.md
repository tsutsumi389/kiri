...
- [x] Phase 14: 意味の事前知識をモデルから借りる（feature `segment`、既定は無効）
  ...
  次フェーズへ送ったもの（Phase 5 のレビューで挙がり、今回は入れていない）:

  - [ ] **推論時にダイジェストを照合しない。** 切り抜きの経路は大きさしか見ない
  ...
  - [ ] `to_constraints` の `grid_index` が内側のループで毎回割り算をしている。
        行ごとに巻き上げれば原寸の画素数ぶんの除算が消える
  - [ ] `auto` の門は背景と主体をもう 1 度測り直している（`decide` →
        `colour_is_hopeless`）。`analyse_background` の結果を持ち回れば、
        `auto` を渡したときの二度手間が無くなる
  - [ ] **推論のピーク RSS 1.4GB を削る。** f16 での実行や、tract の実行計画を
        持ち回して中間テンソルを使い回す道がある。ここが下がらないと
        `batch` は `--segment` を受けられない
...
- [x] Phase 15: 探索を kiri 側に持たせる（`--optimize`）
  ...
  - [x] **致命の数え方を 2 つ直した**（design.md 4.13）。`BBOX_RECOMMENDED` を
        `SUBJECT_TOUCHES_EDGE` と並べて数える（R6 で輪郭誤差 84.00 → 2.22）。
        同じ列で許容量を上げて前景比率が 3 割を超えて落ちた候補は「商品を飲んだ」
        とみなす（`FOREGROUND_COLLAPSE`）
  - [x] **崩れは最終段でも測り直す。** 列の比較でしか見えないので、1 つ回すたびに
        勝者を決めていると原寸でだけ飲む候補を捕まえられない。実写の探索段 1 位
        （tol 60 / bbox）がまさにそれで、1500px では崩れず原寸で前景比率
        0.2037 → 0.0664 になる。最終段は全部回してから選ぶ（そのぶん結果を
        `FINALISTS` 個同時に抱える。24.5MP でピーク約 350MB）
  ...
  - [x] R1〜R7 で `assisted`（3 手ループが到達する設定）と同等以上。7 点中 6 点で
        同等以上、R1 / R5 / R6 は輪郭誤差が半分以下（4.23 → 1.85 / 9.52 → 1.51 /
        5.91 → 2.22）
  - [x] 実写リモコン 24.5MP を bbox 無し・既定値から出発して **14.3 秒**で切る
        （目標 15 秒）。fg 0.2037 / halo 0.0295 / 境界色差 53.8 / 粗さ 0.254 /
        縁汚染 0.061 で、**手で決めた最良設定（`--tolerance 60` + bbox）より良い**
        ——最良値は照明場と matting が入る前のもので、今では商品を 3 分の 2 まで
        飲む（fg 0.0664）
  - [x] 実写キーボード 12MP は 10 候補すべてに致命的な警告が残り、
        `OPTIMIZE_NO_CLEAN_CANDIDATE` が `SUBJECT_TOUCHES_EDGE` を載せて出る
        （8.2 秒）。結果は既定値のままで、探索が何も改善できなかったことを明示する
  ...
  - [ ] **縮小探索の順位は原寸と完全には一致しない。** 実写（4284x5712 → 1500px）
        では原寸の 1 位が探索段で 9 位だった。選ばれたのは原寸の 2 位で、品質の
        重み和は 4.95 対 4.88（1.4% 差）。縮小の無い R1 / R2 / R7 では 3 点とも
        上位 2 つに入った
  ...
  - [ ] **探索段では `--border` と `--feather` が相対的に太くなる。** どちらも
        ...
        足すならここが最初の候補である
  - [ ] `batch` で `optimize: true` を書くと 1 件のピークが約 1.5 倍になる
        （原寸の候補を 2 つ同時に抱えるため）。`--jobs` の数だけ積むので、
        既定の並列度のまま数百点へ流すと確保量で詰まりうる
