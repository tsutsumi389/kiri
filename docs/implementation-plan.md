...
- [x] Phase 15: 探索を kiri 側に持たせ、影を合成する（`--optimize` / `--shadow synth`）
  ...
  - ※ **影の色は 1 色だけ。** 台の色を拾った色付きの影（白い紙の上では
        ...
        自動では決めない
  - [ ] `synth` は原寸のアルファ 1 枚（24.5MP で 24.5MB）と、ぼかしの作業領域
        （同じく 24.5MB）を確保する。**出力の複製 98MB はレビューの修正で消えて
        いる**（上の `[x]` を参照）。キャンバス配置がある場合はキャンバス寸法なので
...
- **推論のピーク RSS 1.4GB を削る**（Phase 14）。f16 での実行か、tract の実行計画の
  持ち回し。**これが下がらない限り次の 1 件は解けない**
- **`batch` が `--segment` を受けない**（Phase 14）。上が前提
- **見立てを持ち回る 3 件** — `auto` の門の二度測り（Phase 14）、探索段の
  `analyse_background` 21 回（Phase 15）、`info --segment` の原寸での再構築
  （Phase 14、24MP で 0.16 秒）。**同じ形の宿題**なので 1 本で片付く
- **確保の 3 件** — `CutoutOptions` の 24MB 複製（Phase 15）、`synth` の
