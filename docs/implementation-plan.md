...
- [x] Phase 11: 空間的な指示
  ...
  - [x] 指示を渡さない経路の出力が 1 バイトも変わらないことを、旧バイナリとの
        ...
        S/R 全 29 点の数値で確認した
  - [x] 不明の帯を matting の作業領域にする → Phase 12
...
- [x] Phase 12: 境界を matting として解く
  ...
  - [x] 塗り直しを 4 パス繰り返す。1 パスで剥がせるのは帯幅ぶんだけだが、実写の
        不織布ではマスクが 20px 級はみ出す。上限を決めるのは診断の側で、5 パス
        目から較正のクリーン側の余裕が 1.96 倍から 1.21 倍へ落ちる
  ...
  - [x] `--matting` / `--smooth-contour` / `--no-reclassify` と `settings` の
        4 項目。`band_min_radius` は**実際に効いた**帯幅の下限で、指定値からは
        読めない
  ...
  - [ ] `rim_contamination` が測る帯（長辺 1000px 換算 3px）に、境界処理が
        届く帯（実寸 3〜10px）が高解像度で足りない。帯の下限・上限を
        `--cleanup` と同じく長辺 1000px 換算で読む
  - [ ] (b)(c) の確保をタイルへ切る（12MP で +43MB、帯の外接矩形ぶん確保して
        いる）
