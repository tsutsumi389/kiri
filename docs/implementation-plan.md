...
順位相関（Spearman ρ ≥ 0.7、25 点）、実写背景シーンの決定性、`diagnose()` が
`cutout()` 全体の 25% を超えないこと。
...
- [x] Phase 10: 境界の欠陥を測れるようにする
  ...
  - [x] 実写背景のベンチ（`tests/real_backgrounds.rs` と
        ...
        27 点の順位相関で固定した（ρ=0.830 / 0.740）。**ベンチが欠陥を見て
        いなければ、以降の改善は測れない**
  - [x] 2 つの測り方を直した。`rim_contamination` は平均色への近さを
        **散らばり（σ）で正規化してから**比べる形にした。前の形は `refine` が
        ...
        実写リモコン（最良設定）は 0.019 → 0.102 / 0.175 → 0.391、
        `RIM_CONTAMINATION_WARN` の両側の余裕は 1.3 倍から 3.5 倍以上になった
  ...
  - [x] `halo_ratio` の「境界近傍か」をチェビシェフ距離変換へ置き換える。
        値は 1 ビットも変わらないまま、12MP で 125ms → 36ms。新しい 2 つを
        足してなお `diagnose()` 全体は着手前より速い
  - [ ] トライマップ入力（`--trimap`）
  - [ ] matting と境界の平滑化
  - [ ] 照明場を持つ背景モデル
