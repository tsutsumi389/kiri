...
- [x] Phase 15: 探索を kiri 側に持たせる（`--optimize`）
  ...
  - [x] 実写キーボード 12MP は 10 候補すべてに致命的な警告が残り、
        ...
        （8.4 秒）。結果は既定値のままで、探索が何も改善できなかったことを明示する
  ...
  - [ ] **R3（布との ΔE が 12 前後の淡色商品）は `assisted` に届かない。** 主体の
        ...
        期待値を現状の値で固定して悪化を検出できる状態にした（`R3_PINNED`）
