...
- [x] Phase 13: 背景を「1 色」ではなく「照明場」として持つ
  ...
  - [x] σ は格子の長辺の 1/32。1/8 では R7 の勾配を追いきれず（輪郭誤差 1.65）、
        1/16 では 2 色の背景の段差を帯へ塗り広げて `subject` が誤って high になる
  - [x] **全解像度の表は作らない。** 格子 + 双線形で都度引く。20MP で
        場の推定 14.3 ms / 約 2MB（予算 30ms / 5MB）
  ...
  - [ ] **R2（照明勾配の不織布）の `defaults` は bbox 無しでは切れない。**
        前景比率 0.502 → 0.388（正解 0.375）、輪郭誤差 121.5 → 100.8、
        rim 正解 0.854 → 0.574 と近づくが、外周接触は解消しない。残っているのは
