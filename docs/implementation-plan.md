...
- [x] Phase 13: 背景を「1 色」ではなく「照明場」として持つ
  ...
  - [ ] **R2（照明勾配の不織布）の `defaults` は bbox 無しでは切れない。**
        前景比率 0.502 → 0.390（正解 0.375）、輪郭誤差 121.5 → 109.4、
        rim 正解 0.854 → 0.608 と近づくが、外周接触は解消しない。残っているのは
        照明ではなく**織り目**で、堤防（4.3）の担当である
...
§5 の `[ ]` は 9 件ある（`※` の 20 件は作業ではないので数えない）。着手の順は
