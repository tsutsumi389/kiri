...
pub struct SubjectReport {
    ...
    /// 主体の最小外接矩形が軸に揃う回転角(度)。**`cutout --rotate` や
    /// `kiri rotate` にそのまま渡せる**（時計回りが正、範囲は (-45, 45]）。
    ///
    ...
    /// 同じ扱いである。`confidence` が `"high"` のときだけ根拠にしてよいのも
    /// 同じ。
    ///
    /// どの角度でも外接矩形の面積が変わらない形（円、辺の多い多角形）では
    /// `null`。**0 ではない**——0 と報告すると「傾いていないと測り切った」に
    /// 読めるが、実際には測れていない。軸に揃った矩形は測り切れているので
    /// `0.0` を返す。
    ///
    /// **分解能は 0.1〜0.5 度。** 主体は縮小版で測るので、輪郭の量子化が
    ...
    /// 超えたときだけ、ここが頭打ちになって食い違う。**`--border` は背景色の
    /// 推定範囲を決める値であって、主体の較正のための値ではない**——帯を
    /// 半分の画像まで広げると、外周 ΔE の分布（主体を拾うしきい値そのもの）が
    /// 商品自身に汚染され、救えない画像が `"high"` と名乗り始める。
    pub border: u32,
...
pub struct LintCheck {
    ...
    /// **読めた事実は返しつつ合否は名乗れない**。単色でない背景の
    /// `background` も同じで、`rgb` と `delta_e` は返る——「背景がベージュで
    /// 白から ΔE 40 外れている」は、均一と言えなくても人が次の一手を選ぶのに
    /// 要る事実である。`status` を見ずに `actual` の有無で合否を読まないこと。
    ///
