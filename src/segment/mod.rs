...
/// | 画像 | 前処理 | 前景比率 | 粗さ | 目視 |
/// |---|---|---|---|---|
/// | キーボード 3024x4032 | letterbox | 42.72% | 0.648 | **右列が欠ける** |
/// | キーボード | stretch | 45.00% | **0.388** | 欠けない |
/// | リモコン 4284x5712 | letterbox | 20.51% | 0.276 | 差なし |
/// | リモコン | stretch | 20.51% | 0.274 | 差なし |
///
...
/// 1 つ増えるのと変わらない。
///
/// **`separability` はこの比較では読めない。** letterbox のほうが高く出る
/// （21.0 対 16.6）が、それは商品の右列を削った結果、境界が銀色の枠と暗い机の
/// あいだに来るからである。**商品を削った側ほど境界の色差は大きく出る。**
///
/// 細長い商品（1:3 以上）で歪みが効いてくる可能性は残っているが、手元の実写
/// 2 枚は両方 3:4 なのでそこは測れていない。`Letterbox` は残してある。
/// docs/design.md 4.12 の表を参照。
